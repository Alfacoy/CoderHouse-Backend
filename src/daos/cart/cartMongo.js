...
export default class CartMongo extends Contenedor {
    ...
    async addToCart(id, pid) {
        try {
            ...
            let isProdInCart = await this.collection.findOne({ _id: id, productos: pid });
            if (isProdInCart) {
                return { status: 'Error', message: `No se puede agregar varias veces el mismo ID.` };
            }
    ...
    async removeFromCart(id, pid) {
        try {
            ...
            let isProdExist = await this.product.collection.findOne({ _id: pid });
            if (!isProdExist) {
                return { status: 'Error', message: 'El producto que eliminar del carrito no es valido.' };
            }
